...
@pytest.mark.parametrize("distro_file", ["osx-64-baseballmetrics-0.1.0-0.tar.bz2"])
def test_distribution_summary_correctly_reads_contents_of_bz2_conda_packages(distro_file):
    ds = _DistributionSummary.from_file(os.path.join(TEST_DATA_DIR, distro_file))

    # should correctly capture the contents:
    #   * 0 directories (conda-build drops empty directories)
    #   * 12 files
    assert len(ds.directories) == 0
    assert ds.num_directories == 0
    assert len(ds.files) == 12
    ...
    expected_file_paths = [
        ...
        "info/paths.json",
        "info/recipe/parent/install-debug-baseballmetrics.sh",
        "info/recipe/parent/install-baseballmetrics.sh",
        "info/recipe/install-baseballmetrics.sh",
        "info/index.json",
